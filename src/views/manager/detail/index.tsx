...
type Desc = Record<
  string,
  {
    ...
    transform?: (val: number | string | boolean) => string | JSX.Element;
  }
>;
...
const detailDescMap: Desc = {
  ...
  avatar: {
    label: '头像',
    transform: (val) => <img src={val as string} alt='头像' style={{ width: 48, height: 48 }} />,
  },
  username: { label: '名称' },
  mobile: { label: '手机号' },
  gender: { label: '性别', transform: (val) => genderMap[val as number] },
  type: { label: '类型', transform: (val) => adminTypeMap[val as number] },
    ...
    try {
      ...
      if (code === ResultEnum.SUCCESS) {
        ...
        for (const [key, value] of Object.entries(result)) {
          detailArr.push({
            value: detailDescMap[key].transform?.(value as string | boolean | number) ?? value,
            ...detailDescMap[key],
          });
        }
